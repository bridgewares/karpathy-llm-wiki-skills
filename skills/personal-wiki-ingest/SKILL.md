...
ingest 的关键动作是：把本次明确 ingest 的来源与 vault 中已有 `wiki/sources/`、`wiki/knowledge/`、`wiki/syntheses/`
放在同一个分析集合中，整体抽取方法论、概念、问题、决策原则、操作经验和跨文档关联。

本 skill 以 `personal-wiki-init` 定义的最小 schema 和 `system/workflow.md` 的 `Page Shapes` 为准：
...
ingest 还需要判断同步强度。用户未指定时，默认使用 **normal ingest**。

| 强度        | 适用场景                                | 必做                                                                 | 不做                              |
|-----------|-------------------------------------|--------------------------------------------------------------------|---------------------------------|
| `minimal` | 用户只要求入库、登记、建立来源追踪，或来源信息很薄           | 移动到 library、更新 raw-index、创建/更新 source page、更新 index/log            | 不主动创建 knowledge；只记录候选           |
| `normal`  | 默认模式；大多数首次 ingest / re-ingest       | minimal 全部内容；更新已有相关 knowledge；必要时创建少量高价值 knowledge；写出 synthesis 候选 | 不强制新建 synthesis；不批量制造 knowledge |
| `deep`    | 用户明确要求深度整理、跨来源综合、专题沉淀，或来源明确改变已有综合结论 | normal 全部内容；可更新或创建 synthesis；可更系统地补双链                              | 不创建无证据支撑的页面，不按名词批量拆页            |

强度判断规则：

- 用户说“只入库 / 先登记 / 建 source page”时，用 `minimal`。
- 用户没有指定时，用 `normal`。
- 用户说“深度整理 / 形成综合 / 写 synthesis / 跨来源分析”时，用 `deep`。
- 批量 ingest 默认仍是 `normal`，但每个来源的 knowledge 创建要更克制。
...
8. **必须建立 Obsidian 关联**
   ...
   - 如果当前来源抽取出了稳定方法论、概念、问题或操作原则，且没有合适 knowledge page，minimal ingest 记录候选，normal / deep ingest 创建最小 knowledge page 并链接。
...
9. **必须做知识链更新**
   ...
   - `wiki/sources/` 是 provenance 层，不是 `raw/sources/` 的镜像；`raw/library/notes/` 也应先有对应 source page，再按内容抽取
     knowledge。
   - 无论来源来自 `raw/library/sources/` 还是 `raw/library/notes/`，都要基于内容提取核心联系，创建或更新必要的 source
     page、knowledge page 和 synthesis 候选。
   - knowledge page 不是 query 的专属产物；normal / deep ingest 应主动创建或更新用于承载方法论、概念、问题、操作原则的
     knowledge page。
   - minimal ingest 可以只在 source page 中记录 knowledge 候选，不主动创建新 knowledge page。
   - synthesis page 是高门槛产物；normal ingest 只检查、链接或提出候选，deep ingest 才默认允许创建或大幅更新 synthesis。
...
- minimal ingest 至少记录 knowledge 候选；normal / deep ingest 创建或更新必要 `wiki/knowledge/`
- normal ingest 至少给出 synthesis 候选或链接已有 synthesis；deep ingest 可更新或创建 `wiki/syntheses/`
- `index.md` 已更新高价值入口
- `log.md` 已追加记录

## 执行流程

source page 不是 ingest 的终点。执行流程必须按同步强度完成闭环：

- minimal：来源入库 -> source page -> 候选记录 -> index/log
- normal：来源入库 -> 整体语料分析 -> source page -> 少量高价值 knowledge -> synthesis 候选 -> 双链验证
- deep：normal -> synthesis 更新或创建 -> 更完整的证据链验证
...
- 关联 knowledge 必须链接本次创建、更新或识别出的相关 knowledge page；minimal ingest 可先记录明确候选。
...
ingest 必须判断是否需要同步 `wiki/knowledge/`。只要来源中存在可复用的方法论、概念、问题、决策原则或操作经验，normal / deep ingest 就应创建或更新 knowledge page；minimal ingest 只记录候选和证据。

同步规则：

- 已有相关 knowledge page：normal / deep ingest 优先更新；minimal ingest 至少在 source page 中链接或记录候选。
- 没有相关 knowledge page：normal / deep ingest 在信息足够稳定、可复用时创建最小 knowledge page；minimal ingest 不主动创建。
...
不允许因为“当前没有现成 knowledge page”就停止关联；应先判断当前强度下是创建最小 knowledge page，还是记录候选。

### 9. 判断 syntheses 关联

默认不强制新建 synthesis，但 normal / deep ingest 必须检查已有 `wiki/syntheses/`，并在 source page 中给出综合关联判断。minimal ingest 可只记录明显候选。

满足以下条件之一时，deep ingest 更新或创建 `wiki/syntheses/`；normal ingest 先链接已有 synthesis 或记录候选，除非用户明确要求创建：
...
如果不创建 synthesis，normal / deep ingest 也要在 source page 的“可能的 syntheses”中说明：

- 可综合的方向
- 相关已有 `[[source]]` / `[[knowledge]]`
- 为什么暂不创建独立 synthesis

这是 normal / deep ingest 的必做流程，不是 query 的前置任务。query 可以深化综合，但 ingest 必须先给出与当前强度相称的候选关联。
