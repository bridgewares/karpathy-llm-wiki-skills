...
| `wiki/sources/` | 来源页。每页对应一个 raw 来源，记录“这个来源说了什么”。 |
...
4. **按需创建目录**
   ...
   - 需要处理原始笔记时，才创建 `raw/inbox/notes/`、`raw/library/notes/`，必要时创建 `wiki/knowledge/`。
...
动作：按 raw `notes` bucket 批量处理，移动或复制到 `raw/library/notes/`，保留 `raw/inbox/notes/`，按需要更新 `wiki/knowledge/`。
